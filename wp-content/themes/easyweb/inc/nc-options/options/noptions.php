...
if ( ! class_exists('NHP_Options') ){
	...
	if(!defined('NHP_OPTIONS_DIR')){
		define('NHP_OPTIONS_DIR', get_template_directory() . '/inc/nc-options/options/');
	}
	
	if(!defined('NHP_OPTIONS_URL')){
		define('NHP_OPTIONS_URL', get_template_directory_uri() . '/inc/nc-options/options/');
	}
...
class NHP_Options{
	...
	*/
	function __construct($sections = array(), $args = array(), $extra_tabs = array()){
		...
		$defaults['menu_title'] = esc_html__('Options', 'nhp-opts');
		$defaults['page_icon'] = 'icon-themes';
		$defaults['page_title'] = esc_html__('Options', 'nhp-opts');
		...
		$defaults['help_sidebar'] = esc_html__(' ', 'nhp-opts');
	...
	*/
	function _enqueue(){
		...
		wp_localize_script('nhp-opts-js', 'nhp_opts', array('reset_confirm' => esc_html__('Are you sure? Resetting will loose all custom values.', 'nhp-opts'), 'opt_name' => $this->args['opt_name']));
		...
		echo '<div class="wrap">';
			...
			echo '<form method="post" action="options.php" enctype="multipart/form-data" id="nhp-opts-form-wrapper">';
				...
				echo '<div id="nhp-opts-header">';
					...
					submit_button(esc_html__('Reset to Defaults', 'nhp-opts'), 'secondary', $this->args['opt_name'].'[defaults]', false);
				...
				echo '</div>';
					...
					if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('nhp-opts-saved') == '1'){
						if(isset($this->options['imported']) && $this->options['imported'] == 1){
							echo '<div id="nhp-opts-imported">'.apply_filters('nhp-opts-imported-text-'.$this->args['opt_name'], wp_kses( __('<strong>Settings Imported!</strong>', 'nhp-opts'), array( 'strong' => array() ) ) ).'</div>';
						}else{
							echo '<div id="nhp-opts-save">'.apply_filters('nhp-opts-saved-text-'.$this->args['opt_name'], wp_kses( __('<strong>Settings Saved!</strong>', 'nhp-opts'), array( 'strong' => array() ) ) ).'</div>';
						}
						delete_transient('nhp-opts-saved');
					}
					echo '<div id="nhp-opts-save-warn">'.apply_filters('nhp-opts-changed-text-'.$this->args['opt_name'], wp_kses( __('<strong>Settings have changed!, you should save them!</strong>', 'nhp-opts'), array( 'strong' => array() ) ) ).'</div>';
					echo '<div id="nhp-opts-field-errors">'.wp_kses( __('<strong><span></span> error(s) were found!</strong>', 'nhp-opts'), array( 'strong' => array(), 'span' => array() ) ).'</div>';
					
					echo '<div id="nhp-opts-field-warnings">'.wp_kses( __('<strong><span></span> warning(s) were found!</strong>', 'nhp-opts'), array( 'strong' => array(), 'span' => array() ) ).'</div>';
					...
					echo '<ul id="nhp-opts-group-menu">';
						...
						if(true === $this->args['show_import_export']){
							echo '<li id="import_export_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="import_export_default"><img src="'.$this->url.'img/admin-import.png" /> <span>'.esc_html__('Import / Export', 'nhp-opts').'</span></a>';
							echo '</li>';
						}//if
						...
						if(true === $this->args['dev_mode']){
							echo '<li id="dev_mode_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="nhp-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default"><img src="'.$this->url.'img/glyphicons/glyphicons_195_circle_info.png" /> <span>'.esc_html__('Dev Mode Info', 'nhp-opts').'</span></a>';
							echo '</li>';
						}//if
				...
				echo '<div id="nhp-opts-main">';
					...
					if(true === $this->args['show_import_export']){
						echo '<div id="import_export_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h3>'.esc_html__('Import / Export Options', 'nhp-opts').'</h3>';
							
							echo '<h4>'.esc_html__('Import Options', 'nhp-opts').'</h4>';
							
							echo '<p><a href="javascript:void(0);" id="nhp-opts-import-code-button" class="button-secondary">Import your options</a> <a href="javascript:void(0);" id="nhp-opts-import-link-button" class="button-secondary">Import template options</a></p>';	
							
							echo '<div id="nhp-opts-import-code-wrapper">';
							
								echo '<div class="nhp-opts-section-desc">';
				
									echo '<p class="description" id="import-code-description">'.apply_filters('nhp-opts-import-file-description',esc_html__('Input your backup file below and hit Import to restore your sites options from a backup.', 'nhp-opts')).'</p>';
							...
							echo '<div id="nhp-opts-import-link-wrapper">';
								...
								echo '<div class="nhp-opts-section-desc">';
									...
									echo '<p class="description" id="import-link-description">'.apply_filters('nhp-opts-import-link-description',esc_html__('Select Template and hit Import to load the options.', 'nhp-opts')).'</p>';
								
								echo '</div>';

								echo '<select id="import-link-value" name="'.$this->args['opt_name'].'[import_link]">
											<option value="'.get_template_directory_uri() . '/inc/dummy-data/default.txt">Default</option>
										</select>';
							...
							echo '<p id="nhp-opts-import-action"><input type="submit" id="nhp-opts-import" name="'.$this->args['opt_name'].'[import]" class="button-primary" value="'.esc_html__('Import', 'nhp-opts').'"> <span>'.apply_filters('nhp-opts-import-warning', esc_html__('WARNING! This will overwrite any existing options, please proceed with caution!', 'nhp-opts')).'</span></p>';
							echo '<div id="import_divide"></div>';
							
							echo '<h4>'.esc_html__('Export Options', 'nhp-opts').'</h4>';
							echo '<div class="nhp-opts-section-desc">';
								echo '<p class="description">'.apply_filters('nhp-opts-backup-description', esc_html__('Here you can copy/download your themes current option settings. Keep this safe as you can use it as a backup should anything go wrong. Or you can use it to restore your settings on this site (or any other site). You also have the handy option to copy the link to yours sites settings. Which you can then use to duplicate on another site', 'nhp-opts')).'</p>';
							echo '</div>';
					...
					if(true === $this->args['dev_mode']){
						echo '<div id="dev_mode_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h3>'.esc_html__('Dev Mode Info', 'nhp-opts').'</h3>';
							echo '<div class="nhp-opts-section-desc">';
