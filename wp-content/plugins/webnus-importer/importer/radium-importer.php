 ...
 */
 ...
 if ( !class_exists( 'Radium_Theme_Importer' ) ) {
	...
	class Radium_Theme_Importer {
		 ...
		 * @var array
		 */
		public $flag_as_imported = array( 'content' => false, 'menus' => false, 'sliders' => false, 'reading_settings' => false, 'options' => false, 'widgets' =>false );
	    ...
	    public function __construct() {
	        ...
	        $this->slider_file 			= $this->demo_files_path . $this->slider_file_name;
	    ...
	    public function add_admin() {
	        ...
	        add_submenu_page('themes.php', "Webnus Importer", "Webnus Importer", 'switch_themes', 'radium_demo_installer', array($this, 'demo_installer'));
    	...
    	public function intro_html() {
			...
			<div style="background-color: #F5FAFD; margin:10px 0;padding: 5px 10px;color: #0C518F;border: 2px solid #CAE0F3; clear:both; width:98%; line-height:18px;">
			    ...
			    allow you to quickly edit everything instead of creating content from scratch. When you import the data following things will happen:</p>
			      ...
			      <ul style="padding-left: 20px;list-style-position: inside;list-style-type: square;}">
			          ...
			          <li>Images will be downloaded from our server, these images are <strong>copyrighted</strong> and are for demo use only .</li>
			          <li>Please click import only once and wait, it can take a <strong style="color: red;">couple of minutes</strong></li>
			      </ul>
			 </div>

			 <div style="background-color: #F5FAFD; margin:10px 0 40px;padding: 5px 10px;color: #0C518F;border: 2px solid #CAE0F3; clear:both; width:98%; line-height:18px;"><p class="tie_message_hint" style="color: #77da55;font-weight: 600;">Before you begin, make sure all the required plugins are activated.</p></div>

			 <?php

			 if( !empty($this->imported_demos) ) { ?>

			  	<div style="background-color: #FAFFFB; margin:10px 0 40px;padding: 5px 10px;color: #8AB38A;border: 2px solid #a1d3a2; clear:both; width:98%; line-height:18px;">
	    ...
	    public function demo_installer() {
	       ...
	       <div class="radium-importer-wrap" data-demo-id="1"  data-nonce="<?php echo wp_create_nonce('radium-demo-code'); ?>" style="width:99%;">
			        	...
			        	<div class="theme-browser rendered">
							<div class="themes">
								<?php foreach ($this->content_demos as $key => $value) { ?>
									<div class="theme" style="background-color: #f8f8f8;">
										<label class="theme-screenshot" for="<?php echo $value; ?>">
											<img src="<?php echo plugins_url( 'webnus-importer' ); ?>/demo-files/<?php echo $value; ?>/screen-image.jpg">
										</label>
										<input id="<?php echo $value; ?>" type="radio" value="<?php echo $value; ?>" name="demo-select" style="margin-left: 20px;">
										<h3 class="demo-name" style="display: inline-block;"><?php echo $key; ?></h3>
									</div>
								<?php } ?>
							</div>
						</div>
		        	<?php } ?>
					<p style="font-family: tahoma; font-size: 15px; text-align: center; background: #FCE6E9; border: 2px solid #ED9AA3; border-radius: 3px; display: block; clear: both; padding: 15px;"><span style="color: red; font-weight: bold;">NOTE :</span> Before starting to import, read <a style="font-weight: bold;" href="https://webnus.ticksy.com/article/4415" target="_blank" rel="nofollow">What are best WordPress/ PHP settings?</a> accurately.<br>
					import processing needs configs which has been mentioned in top article.</p>
	
		          	<input type="hidden" name="demononce" value="<?php echo wp_create_nonce('radium-demo-code'); ?>" />
		          	<input onclick="webnus_importer_alert()" name="reset" class="panel-save button-primary radium-import-start" type="submit" value="<?php echo $button_text ; ?>" style="text-align: center; width: 178px; height: auto; margin: auto; display: block; padding: 8px 15px; font-size: 14px;"/>
		          	<input type="hidden" name="action" value="demo-data" />

					<script>
						function webnus_importer_alert() {
							alert("WARNING :\n\nImporting demo will take about 5 till 15 minutes to import all theme options, pages, posts, sliders, widgets, sidebars and page options. exactly like demo.\n\n************************************************************************\n\nIMPORTANT NOTES : \n\n• Memory Limit of 128 MB and max execution time (php time limit) of 300 seconds.\n\n• All plugins must install and activated before import.\n\n");
						}
					</script>
		          	...
		          	<?php 
		          		$this->theme_options_file 	= apply_filters('radium_theme_importer_theme_options_file', trailingslashit($this->demo_files_path) . trailingslashit($demo_selected) . $this->theme_options_file_name);
				        $this->widgets 				= apply_filters('radium_theme_importer_widgets_file', trailingslashit($this->demo_files_path) . trailingslashit($demo_selected) . $this->widgets_file_name);
				        $this->slider_file 			= trailingslashit($this->demo_files_path) . trailingslashit($demo_selected) . $this->slider_file_name;
				        $this->content_demo 		= apply_filters('radium_theme_importer_content_demo_file', trailingslashit($this->demo_files_path) . trailingslashit($demo_selected) . $this->content_demo_file_name);
		          	?>

					<div class="radium-importer-message clear">
				        <?php if( 'demo-data' == $action && check_admin_referer('radium-demo-code' , 'demononce') && $demo_selected ){
		...
		public function process_imports( $content = true, $options = true, $widgets = true) {
			...
			if ( $content && !empty( $this->content_demo ) && is_file( $this->content_demo ) ) {
				...
				$this->set_demo_menus();
				$this->set_demo_reading_settings();
			}
			...
			if ( !empty( $this->slider_file ) && is_file( $this->slider_file ) ) {
				$this->process_slider_import_file( $this->slider_file );
			}
	    ...
	    public function set_demo_theme_options( $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				if( $this->theme_options_framework == 'radium') {
					...
					$data = maybe_unserialize( trim( $data ) );
		 ...
		 * @since 0.0.1
		 */
	    function process_slider_import_file( $file ) {
			...
			$this->flag_as_imported['sliders'] = true;
